document.addEventListener('DOMContentLoaded', () => {
    checkAuth();
    setupFileInput();
    setupDragAndDrop(); // Nouvelle fonction Drag & Drop
});
...
// Nouvelle fonction Drag & Drop
function setupDragAndDrop() {
    const dropZone = document.body;
    
    // Création de l'overlay visuel
    ...
    overlay.innerHTML = `
        <div class="border-4 border-dashed border-white/50 rounded-3xl p-12 flex flex-col items-center animate-pulse">
            <i class="fa-solid fa-cloud-arrow-up text-8xl mb-4"></i>
            <h2 class="text-4xl font-bold font-display">DÉPOSEZ ICI</h2>
            <p class="text-xl mt-2">pour uploader instantanément</p>
        </div>
    `;
    ...
    // On utilise dragleave sur l'overlay pour éviter le clignotement
...
function handleFileSelect(file) {
    if (!file) return;
    const cat = localStorage.getItem('activeFilter') || 'Documents'; // On prend le filtre actif par défaut
    
    // Vérification type (Optionnel, on peut être souple ici et changer la catégorie auto)
    let autoCat = cat;
    ...
    // On ouvre la modale pré-remplie
    openModal(true); // true = mode drop
    ...
    document.getElementById('imgCategory').value = autoCat; // Catégorie auto-détectée
...
// --- GESTION SÉLECTION MULTIPLE (Inchangé) ---
    ...
    filtered.forEach(p => {
        ...
        div.innerHTML = `
            <div onclick="${clickAction}" class="w-full h-full cursor-pointer">
                ...
                ${isSelectionMode ? `<div class="absolute top-2 right-2 w-6 h-6 rounded-full border-2 ${isSel ? 'bg-blue-500 border-blue-500' : 'bg-white/20 border-white'} flex items-center justify-center transition-colors">${isSel ? '<i class="fa-solid fa-check text-white text-xs"></i>' : ''}</div>` : ''}
            </div>
...
function openModal(isDrop = false) {
    ...
    // Si c'est pas un drop, on reset les valeurs par défaut
    if (!isDrop) {
