...
document.addEventListener('DOMContentLoaded', () => {
    // Migration pour les anciennes ventes
...
// --- CORRECTIF CRAYON (PRIX NÉGOCIÉ) ---
window.editCartPrice = (id) => {
    // UTILISATION DE '==' AU LIEU DE '===' POUR EVITER LES ERREURS DE TYPE
    const p = products.find(x => x.id == id); 
    if (!p) return alert("Produit introuvable");

    const currentPrice = cartPrices[id] !== undefined ? cartPrices[id] : p.price;
    negotiatingId = id;
    ...
    setTimeout(() => {
        const input = document.getElementById('negotiated-price');
        input.focus();
        input.select();
    }, 100);
}
...
window.confirmPayment = () => {
    ...
    if (inputVal > pendingSale.total) return alert("Le montant ne peut pas dépasser le total !");
