...
let pendingSale = null; // Pour les dettes
let chartTrend = null;
let chartPie = null;
...
document.addEventListener('DOMContentLoaded', () => {
    // Migration données (si anciennes ventes sans statut)
...
function renderCharts() {
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        last7Days[dateStr] = 0; daysLabels.push(dateStr.slice(0, 5));
    }
    sales.forEach(s => {
        const sDate = new Date(s.id).toLocaleDateString();
        if (last7Days[sDate] !== undefined) last7Days[sDate] += (s.amountPaid || 0);
    });
    ...
    if (ctxTrend) {
        ...
        chartTrend = new Chart(ctxTrend.getContext('2d'), {
            type: 'line',
            data: { labels: daysLabels, datasets: [{ label: 'Cash', data: Object.values(last7Days), borderColor: '#2563eb', backgroundColor: 'rgba(37, 99, 235, 0.1)', borderWidth: 3, tension: 0.4, fill: true, pointRadius: 4 }] },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { display: false }, x: { ticks: { display: true } , grid: { display: false } } } }
        });
    }
...
window.editCartPrice = (id) => { const p = products.find(x => x.id === id); const currentPrice = cartPrices[id] || p.price; negotiatingId = id; document.getElementById('price-modal-product').textContent = p.name; document.getElementById('negotiated-price').value = currentPrice; document.getElementById('priceModal').classList.remove('hidden'); setTimeout(() => document.getElementById('negotiated-price').focus(), 100); }
window.saveNegotiatedPrice = () => { if (!negotiatingId) return; const inputVal = document.getElementById('negotiated-price').value; const newPrice = parseInt(inputVal); if (!isNaN(newPrice) && newPrice >= 0) { cartPrices[negotiatingId] = newPrice; updateCartUI(); closePriceModal(); } else { alert("Prix invalide"); } }
window.closePriceModal = () => { document.getElementById('priceModal').classList.add('hidden'); negotiatingId = null; }
...
function updateHistoryUI() {
    ...
    const debtsPending = totalAllTime - totalCashIn;

    let html = `<div class="grid grid-cols-2 gap-3 mb-4"><div class="bg-slate-800 rounded-xl p-3 border border-slate-700"><p class="text-[9px] text-slate-400 font-bold uppercase mb-1">Caisse Réelle</p><p class="text-xl font-mono font-bold text-green-400">${totalCashIn.toLocaleString()} <span class="text-xs">F</span></p></div><div class="bg-slate-800 rounded-xl p-3 border border-slate-700"><p class="text-[9px] text-slate-400 font-bold uppercase mb-1">Dettes Dehors</p><p class="text-xl font-mono font-bold text-orange-400">${debtsPending.toLocaleString()} <span class="text-xs">F</span></p></div></div>`;

    if (sales.length === 0) { html += `<div class="text-center mt-10 opacity-50"><i class="fa-solid fa-file-invoice text-4xl text-slate-600 mb-2"></i><p class="text-slate-500 text-sm">Aucune vente.</p></div>`; } else {
        html += sales.map(s => {
            ...
            const remaining = s.total - (s.amountPaid || 0); const isDebt = remaining > 0;
            const borderClass = isDebt ? 'border-l-orange-500' : 'border-l-blue-500';
            const statusBadge = isDebt ? `<div class="mt-2 flex justify-between items-center bg-orange-50 dark:bg-orange-900/20 p-2 rounded-lg"><span class="text-xs font-bold text-orange-600 dark:text-orange-400">Reste: ${remaining.toLocaleString()} F</span><button onclick="settleDebt(${s.id})" class="bg-orange-500 text-white text-[10px] font-bold px-3 py-1.5 rounded shadow-sm hover:bg-orange-600 transition">SOLDER</button></div>` : '';
            return `<div class="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-100 dark:border-transparent border-l-4 ${borderClass} mb-3"><div class="flex justify-between mb-1"><span class="font-bold text-sm text-slate-600 dark:text-slate-400">${s.date.split(',')[0]}</span><div><span class="font-bold text-slate-800 dark:text-slate-200">${s.total.toLocaleString()} F</span>${surplusText}</div></div><div class="text-sm text-slate-700 dark:text-white mb-1"><span class="font-bold">${s.client}</span> (${s.items.length} articles)</div>${statusBadge}</div>`;
        }).join('');
    }
    list.innerHTML = html;
}
