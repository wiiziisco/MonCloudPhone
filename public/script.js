...
function logoutUser() {
    showConfirmModal("Déconnexion", "Fermer la session ?", () => {
...
function handleFileSelect(file) {
    ...
    openModal(true); document.getElementById('uploadPlaceholder').classList.add('hidden'); document.getElementById('filePreviewInfo').classList.remove('hidden'); document.getElementById('previewName').textContent = file.name; document.getElementById('imgTitle').value = file.name.split('.')[0]; document.getElementById('imgCategory').value = autoCat;
    ...
    const reader = new FileReader(); reader.onload = (ev) => document.getElementById('base64String').value = ev.target.result;
...
function saveNote() {
    const content = document.getElementById('noteContent').value; let title = document.getElementById('noteTitle').value.trim() || `Note_${Date.now()}.txt`; if (!title.endsWith('.txt')) title += '.txt';
    ...
    const photos = JSON.parse(localStorage.getItem('my_gallery_data') || '[]'); photos.unshift({ id: Date.now(), image: base64, title: title.replace('.txt',''), category: 'Documents', type: 'text', date: new Date().toLocaleDateString() });
    localStorage.setItem('my_gallery_data', JSON.stringify(photos)); closeNoteModal(); showNotification("Note cryptée sauvegardée !", "success"); localStorage.setItem('activeFilter', 'Documents'); loadGallery();
}
...
function loadGallery() {
    const gallery = document.getElementById('gallery'); const currentFilter = localStorage.getItem('activeFilter') || 'Documents'; const photos = JSON.parse(localStorage.getItem('my_gallery_data') || '[]');
    ...
    if(!document.getElementById('btn-select-mode')) { const selBtn = document.createElement('button'); selBtn.id = 'btn-select-mode'; selBtn.className = "px-4 py-1.5 rounded-full text-xs font-bold transition whitespace-nowrap border ml-2 bg-slate-800 border-slate-700 text-gray-400"; selBtn.onclick = toggleSelectionMode; filterContainer.appendChild(selBtn); }
    const selBtn = document.getElementById('btn-select-mode'); selBtn.innerHTML = isSelectionMode ? '<i class="fa-solid fa-xmark mr-1"></i> Annuler' : '<i class="fa-regular fa-square-check mr-1"></i> Sélect.';
    const existingDel = document.getElementById('btn-delete-selected'); if(existingDel) existingDel.remove();
    if(isSelectionMode && selectedItems.size > 0) { const delBtn = document.createElement('button'); delBtn.id = 'btn-delete-selected'; delBtn.className = "px-4 py-1.5 rounded-full text-xs font-bold transition whitespace-nowrap border ml-2 bg-red-600 text-white border-red-600 animate-pulse"; delBtn.innerHTML = `<i class="fa-solid fa-trash mr-1"></i> (${selectedItems.size})`; delBtn.onclick = deleteSelectedItems; filterContainer.appendChild(delBtn); }
    gallery.innerHTML = '';
    const filtered = photos.filter(p => p.category === currentFilter && p.title.toLowerCase().includes(searchTerm));
    if (!isSelectionMode && searchTerm === "") { const addDiv = document.createElement('div'); addDiv.className = "aspect-square rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/40 hover:bg-white dark:hover:bg-slate-800 hover:border-blue-500 transition flex flex-col items-center justify-center cursor-pointer group"; addDiv.innerHTML = `<div class="w-12 h-12 rounded-full bg-slate-200 dark:bg-slate-700 group-hover:bg-blue-600 flex items-center justify-center mb-2 transition shadow-lg"><i class="fa-solid fa-plus text-blue-500 dark:text-blue-400 group-hover:text-white text-xl"></i></div><span class="text-[10px] font-bold text-slate-500 dark:text-gray-500 group-hover:text-slate-800 dark:group-hover:text-white uppercase tracking-wider">Ajouter</span>`; addDiv.onclick = () => openModal(false); gallery.appendChild(addDiv); }
    filtered.forEach(p => {
        ...
        let visual = ''; if (p.type === 'image') visual = `<img src="${p.image}" class="w-full h-full object-cover transition ${isSelectionMode ? 'opacity-70' : 'opacity-90 group-hover:opacity-100'}">`; else { let icon = 'fa-file'; let col = 'text-gray-500'; if (p.type === 'video') { icon = 'fa-video'; col = 'text-red-500'; } if (p.type === 'audio') { icon = 'fa-music'; col = 'text-pink-500'; } if (p.type === 'apk') { icon = 'fa-android'; col = 'text-green-500'; } if (p.type === 'zip') { icon = 'fa-file-zipper'; col = 'text-yellow-500'; } if (p.type === 'text') { icon = 'fa-file-code'; col = 'text-green-500'; } visual = `<div class="w-full h-full flex items-center justify-center ${isSel ? 'bg-transparent' : 'bg-slate-50 dark:bg-slate-900'}"><i class="fa-solid ${icon} ${col} text-4xl"></i></div>`; }
        const clickAction = isSelectionMode ? `toggleItemSelection(${p.id})` : `openFullscreen('${p.id}')`; div.innerHTML = `<div onclick="${clickAction}" class="w-full h-full cursor-pointer">${visual}<div class="absolute bottom-0 left-0 w-full bg-white/90 dark:bg-black/60 backdrop-blur-sm p-1.5 text-center border-t border-slate-100 dark:border-transparent"><p class="text-[10px] font-bold text-slate-700 dark:text-white truncate">${p.title}</p></div>${isSelectionMode ? `<div class="absolute top-2 right-2 w-6 h-6 rounded-full border-2 ${isSel ? 'bg-blue-500 border-blue-500' : 'bg-white/20 border-white'} flex items-center justify-center transition-colors">${isSel ? '<i class="fa-solid fa-check text-white text-xs"></i>' : ''}</div>` : ''}</div>${!isSelectionMode ? `<button onclick="deletePhoto(${p.id})" class="absolute top-1 right-1 bg-red-500 text-white w-6 h-6 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition shadow"><i class="fa-solid fa-trash text-[10px]"></i></button>` : ''}`; gallery.appendChild(div);
    });
}
...
function deletePhoto(id) { showConfirmModal("Supprimer ?", "Sûr ?", () => { const photos = JSON.parse(localStorage.getItem('my_gallery_data') || '[]'); const newPhotos = photos.filter(p => p.id !== id); localStorage.setItem('my_gallery_data', JSON.stringify(newPhotos)); loadGallery(); showNotification("Élément supprimé", "success"); }); }
