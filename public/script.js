...
function checkAuth() { if (localStorage.getItem('is_logged_in') === 'true') { document.documentElement.classList.add('logged-in'); showApp(); } }
function toggleAuthMode() { isRegistering = !isRegistering; document.getElementById('auth-btn').innerText = isRegistering ? "S'INSCRIRE" : "ENTRER"; document.getElementById('auth-toggle-text').innerText = isRegistering ? "Retour à la connexion" : "Première visite ? Créer un accès"; }
function handleAuth() {
    ...
    if (isRegistering) {
        ...
        localStorage.setItem('user_account', JSON.stringify({ u, p })); localStorage.setItem('is_logged_in', 'true'); showNotification("Compte créé !", "success"); showApp();
    } else {
        const acc = JSON.parse(localStorage.getItem('user_account'));
        if (acc && u === acc.u && p === acc.p) { localStorage.setItem('is_logged_in', 'true'); showApp(); } else { showNotification("Erreur d'identification", "error"); }
    }
}
function showApp() { document.getElementById('auth-screen').classList.add('hidden'); document.getElementById('app-wrapper').classList.remove('hidden'); if (!localStorage.getItem('activeFilter') || localStorage.getItem('activeFilter') === 'all') { localStorage.setItem('activeFilter', 'Documents'); } loadGallery(); }
...
function toggleSearchBar() {
    const title = document.getElementById('headerTitle'); const input = document.getElementById('searchInput'); const icon = document.getElementById('searchIcon');
    if (input.classList.contains('opacity-0')) { input.classList.remove('opacity-0', 'pointer-events-none', 'scale-95'); input.classList.add('opacity-100', 'scale-100'); input.focus(); title.classList.remove('opacity-100', 'scale-100'); title.classList.add('opacity-0', 'scale-95'); icon.className = "fa-solid fa-xmark text-lg"; }
    else { input.classList.remove('opacity-100', 'scale-100'); input.classList.add('opacity-0', 'pointer-events-none', 'scale-95'); input.value = ""; handleSearch(""); title.classList.remove('opacity-0', 'scale-95'); title.classList.add('opacity-100', 'scale-100'); icon.className = "fa-solid fa-magnifying-glass text-lg"; }
}
...
function loadGallery() {
    ...
    const selBtn = document.createElement('button'); selBtn.id = 'btn-select-mode'; selBtn.className = `px-4 py-1.5 rounded-full text-xs font-bold transition whitespace-nowrap border ml-2 ${isSelectionMode ? 'bg-orange-500 text-white border-orange-500' : 'bg-white dark:bg-slate-800 text-slate-500 dark:text-gray-400 border-slate-200 dark:border-slate-700'}`; selBtn.innerHTML = isSelectionMode ? '<i class="fa-solid fa-xmark mr-1"></i> Annuler' : '<i class="fa-regular fa-square-check mr-1"></i> Sélect.'; selBtn.onclick = toggleSelectionMode; filterContainer.appendChild(selBtn);
    if (isSelectionMode && selectedItems.size > 0) { const delBtn = document.createElement('button'); delBtn.id = 'btn-delete-selected'; delBtn.className = "px-4 py-1.5 rounded-full text-xs font-bold transition whitespace-nowrap border ml-2 bg-red-600 text-white border-red-600 animate-pulse"; delBtn.innerHTML = `<i class="fa-solid fa-trash mr-1"></i> Supprimer (${selectedItems.size})`; delBtn.onclick = deleteSelectedItems; filterContainer.appendChild(delBtn); }
    ...
    if (filtered.length === 0 && searchTerm !== "") { gallery.innerHTML = `<div class="col-span-full text-center text-slate-400 mt-8 italic">Aucun résultat pour "${searchTerm}"</div>`; }
...
function openModal(isDrop = false) {
    const modal = document.getElementById('uploadModal'); const cat = localStorage.getItem('activeFilter') || 'Documents'; const input = document.getElementById('fileInput'); const instr = document.getElementById('uploadInstruction');
